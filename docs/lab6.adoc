...
. Subscribe to the `kitchen_orders` topic. On a `KitchenOrderPrepStartedEvent`, create a `Pizza` for each `KitchenOrder.Pizza`, and add it to the repository. Then `startPrep()` on the `Pizza`.
